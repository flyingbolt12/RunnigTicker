...
<html:form action="saveTimerContent.do?parameter=createTimerContent" enctype="multipart/form-data">
	<table border="0" cellspacing="1" width="100%" style="font-family: Tahoma; font-size: 10pt" height="102" align="center">
		<tr>
			<td bgcolor="#808080" colspan="2">
				...
				<p align="center">
					<b><font color="#FFFFFF">Create a new Timer Email Content</font></b>
			</td>
		<tr>
			<td bgcolor="#EEEEEE">Timer Content Name</td>
			<td height="18" bgcolor="#EEEEEE"><html:text property="contentName" styleClass="BusinessTextBox" tabindex="1" size="53" styleId="contentName"
					onblur="verifyText(this.value)"></html:text> <font color="#000000"><span id="emailNameAvailabilityCheckMsg"></span></font>
		</tr>

		<tr>
			<td bgcolor="#EEEEEE">Email Subject</td>
			<td height="18" bgcolor="#EEEEEE">
				
				<html:messages id="err_name" property="subject"> <%	errMsg = err_name;  isRreadOnly = false; %>  </html:messages> 
				
				<%  if (isRreadOnly) { %>				
					<html:text property="subject" styleClass="BusinessTextBox" tabindex="2" size="53" /> 
				<% } else { %> 
					<html:text property="subject" styleClass="BusinessTextBox" tabindex="2" size="53">
					</html:text> <%=errMsg%> 
		...
		<tr>
			<td bgcolor="#F4F4F4" height="18">Email Content</td>
			<td bgcolor="#F4F4F4" height="18">
			
				<html:messages id="err_name" property="emailContent"> <%	errMsg = err_name;  isRreadOnly = false; %>  </html:messages> 
				
				<%  if (isRreadOnly) { %>				
					<html:textarea property="emailContent" styleClass="BusinessTextBox" tabindex="3" cols="50" rows="2"></html:textarea>
				<% } else { %> 
					<html:textarea property="emailContent" styleClass="BusinessTextBox" tabindex="3" cols="50" rows="2"></html:textarea> <%=errMsg%> 
				<%  errMsg = ""; isRreadOnly = true; }  %>				
		...
		<tr>
			<td bgcolor="#EEEEEE" height="18">Email Attachment</td>
			<td bgcolor="#EEEEEE" height="18"><font color="#FFFFFF" face="Tahoma" size="2"></font> <c:choose>
					<c:when test="${isTimerContentToUpdate == 'yes'}">
			* Note : Attachments update not supported. Delete this content and create a new one if you need.
			</c:when>
					<c:otherwise>
						<html:file property="attachment" styleClass="BusinessTextBox" tabindex="3" />
					</c:otherwise>
				</c:choose></td>
		</tr>

		<tr>
			<td bgcolor="#F4F4F4" height="18">Email Signature</td>
			<td bgcolor="#F4F4F4" height="18">
				<html:messages id="err_name" property="signature"> <%	errMsg = err_name;  isRreadOnly = false; %>  </html:messages> 
				<%  if (isRreadOnly) { %>				
					<html:textarea property="signature" styleClass="BusinessTextBox" tabindex="5" cols="50" rows="2"></html:textarea>
				<% } else { %> 
					<html:textarea property="signature" styleClass="BusinessTextBox" tabindex="5" cols="50" rows="2"></html:textarea> <%=errMsg%> 
				<%  errMsg = ""; isRreadOnly = true; }  %>				
			</td>
		</tr>
