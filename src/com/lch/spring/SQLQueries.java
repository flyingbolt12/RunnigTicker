...
public interface SQLQueries {
	...
	static final String LISTALLEMPLOYEESKILLS = " select *," + "(SELECT concat('(T)',totalHrsSubmitted,' / ',' (R) ',totalRegularHrs,' / ', ' (O) ',totalOvertimeHrs,' / ','(H) ',totalHolidayHrs,'  --  ',startWeekDate,'-',endWeekDate) FROM weeklyhrssummary where weeklyHrsSummaryId=(select max(weeklyHrsSummaryId) from weeklyhrssummary where userId=u.idUser)) as recentHrs, " + "(select concat(address1,', ',address2,', ',city,', ',state,', ',zipcode)  from addressinfo a where a.idAddressInfo=up.clientAddressId) as cleintAddress, " + "(select concat(address1,', ',address2,', ',city,', ',state,', ',zipcode)  from addressinfo a where a.idAddressInfo=up.myAddressId) as personalAddress " + "from (users u, userpersonaldata up, userclientslist cl) left join skillTags i on (u.iduser=i.userId) where u.personalDetailsId=up.iduserData and u.businessId=? and u.role='MEMBER' AND cl.clientId=u.clientId";
	...
	static final String LIST_ALL_NON_DELETED_ADMIN_TASKS = "select * from admintasks where status != 3";
	// ADMIN TASKS - END
