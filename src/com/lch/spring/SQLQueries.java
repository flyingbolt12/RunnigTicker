...
public interface SQLQueries {
	...
	static final String INSERTHRS = "insert into weeklyhrs (  weekNumber ,  weekStartDate,  weekEndDate,  SUNDAY,  MONDAY,  TUESDAY,  WEDNESDAY,  THURSDAY,  FRIDAY,  SATURDAY,  submissionType,  year,  idUser,  idBusiness,  total,  month, submissionConfiguredTo, submissionFor) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
	static final String INSERT_WEEKLY_HRS_SUMMARY = "insert into weeklyhrssummary (  userId ,  businessId,  month,  year,  totalHrsSubmitted,  status,  totalRegularHrs,  totalOvertimeHrs,  submittedDate, clientId, submissionConfiguredTo, submissionFor,totalHolidayHrs,startWeekDate,endWeekDate) values (:userId,:businessId,:month,:year,:totalHrsSubmitted,:status,:totalRegularHrs,:totalOvertimeHrs,:submittedDate, :clientId, :submissionConfiguredTo, :submissionFor, :totalHolidayHrs, :startWeekDate, :endWeekDate)";
	...
	static final String INSERTUSERPERSONALDATA = "INSERT INTO `userpersonaldata` (`firstName`,`lastName`,`middleName`,`clientAddressId`,`myAddressId`,`contactEmail`,`phoneNumber`,`fatherName`,`countryCitizenship`,`homeCountryAddressId`,`profilePath`,`gendar`, `dob`) VALUES (:firstName,:lastName,:middleName,:clientAddressId,:myAddressId,:contactEmail,:phoneNumber,:fatherName,:countryCitizenship,:homeCountryAddressId,:profilePath,:gendar, :dobDate)";
	...
	static final String INSERTUSER_WITH_APPROVAL_STATUS = "INSERT INTO `users` (`login`,`password`,`isPwdChangeRequired`,`expairyDate`,`registeredDate`,`businessId`,`role`,`personalDetailsId`,`clientId`,`approvalStatus`,`timeSheetConfiguredTo`) VALUES (:login,MD5(:password),:isPwdChangeRequired,:expairyDate,:registeredDate,:businessId,:role,:personalDetailsId,:clientId,:approvalStatus, :timeSheetConfiguredTo)";
	...
	static final String USER_COUNT_BY_DEMO_EMPLOYER = "select count(1) from users u where role='ADMIN' and login='employer'";
	...
	static final String BUSINESS_DETAILS_BY_BUSINESS_USER_ITSELF = "select l.*,u.*, up.* from lch_business l, users u ,userpersonaldata up where l.businessId=u.businessId and u.businessId = ? and u.role = 'ADMIN' and up.iduserData = u.personalDetailsId and u.idUser= ?";
	...
	static final String EMPLOYEE_TIMESHEET_PENDING_APPROVALS_4_ADMIN = "SELECT upd.firstName, upd.middleName, upd.lastName, clientsList.clientName clientWorkingFor,w.supportingDocIds,w.totalHrsSubmitted submittedHrs, w.weeklyHrsSummaryId, w.month, w.year, w.totalRegularHrs, w.totalHolidayHrs, w.totalOvertimeHrs, w.startWeekDate, w.endWeekDate, w.submissionFor, w.status, u.idUser userId, w.submissionConfiguredTo timeSheetConfiguredTo FROM weeklyhrssummary w, users u, userpersonaldata upd, userclientslist clientsList where w.businessId=? and u.idUser=w.userId and upd.iduserData=u.personalDetailsId and w.status='PENDING' and clientsList.clientId=u.clientId";
	static final String LIST_EMPLOYEE_TIMESHEETS_FOR_ADMIN = "SELECT upd.firstName, upd.middleName, upd.lastName, clientsList.clientName clientWorkingFor,w.supportingDocIds,w.totalHrsSubmitted submittedHrs, w.weeklyHrsSummaryId, w.month, w.year, w.totalRegularHrs, w.totalHolidayHrs, w.totalOvertimeHrs, w.startWeekDate, w.endWeekDate, w.submissionFor, w.status, u.idUser userId, u.timeSheetConfiguredTo timeSheetConfiguredTo FROM weeklyhrssummary w, users u, userpersonaldata upd, userclientslist clientsList where w.businessId=? and u.idUser=w.userId and upd.iduserData=u.personalDetailsId and clientsList.clientId=u.clientId and u.idUser=? and w.status<>'PENDING'";
	...
	static final String FETCH_UPDATE_CLIENT = "SELECT clientsList.clientName,addressinfo.*,clientsList.startDate	FROM	userclientslist clientsList ,addressinfo addressInfo, userpersonaldata pdata, users users where pdata.iduserdata = users.personalDetailsId	and pdata.clientAddressId = addressInfo.idAddressInfo	and users.idUser = ?	and users.clientId = clientsList.clientId	and users.businessId =?";
	...
	static final String FETCH_BUSINESS_ADMINS = "SELECT * from userpersonaldata up,users u where u.personalDetailsId=up.iduserData and u.role='ADMIN' and u.businessId=? and approvalStatus not in (3)";
